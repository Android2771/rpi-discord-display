...
def bicker_counter_add(btn):
  response = requests.get('https://andrewbuhagiar.com:8443/counter/bickercount')
  if response.status_code == 200:
    requests.get(f'http://localhost:7000/draw?author=Bicker%20Counter&text={response.text}%20bickers!')  
  else:
    requests.get(f'http://localhost:7000/draw?author=Bicker%20Counter&text=Problem occured!')  
    
  return 'OK'
    
def bicker_counter_no_add(btn):
  response = requests.get('https://andrewbuhagiar.com:8443/counter/bickercount?increment=false')
  if response.status_code == 200:
    requests.get(f'http://localhost:7000/draw?author=Bicker%20Counter&text={response.text}%20bickers!')  
  else:
    requests.get(f'http://localhost:7000/draw?author=Bicker%20Counter&text=Problem occured!')  
...
btn3.when_pressed = bicker_counter_add
btn4.when_pressed = bicker_counter_no_add
